...
fn fuzz_is_ascii(data: &[u8]) {
    assert_eq!(
        encoding_rs2::mem::is_ascii(data),
        safe_encoding_rs_mem::is_ascii(data)
    );
}

fn fuzz_is_basic_latin(data: &[u16]) {
    assert_eq!(
        encoding_rs2::mem::is_basic_latin(data),
        safe_encoding_rs_mem::is_basic_latin(data)
    );
}

fn fuzz_is_utf8_latin1(data: &[u8]) {
    assert_eq!(
        encoding_rs2::mem::is_utf8_latin1(data),
        safe_encoding_rs_mem::is_utf8_latin1(data)
    );
}
...
fn fuzz_is_str_latin1(data: &[u8]) {
    if let Ok(s) = std::str::from_utf8(data) {
        assert_eq!(
            encoding_rs2::mem::is_str_latin1(s),
            safe_encoding_rs_mem::is_str_latin1(s)
        );
    }
}
...
fn fuzz_is_utf16_latin1(data: &[u16]) {
    assert_eq!(
        encoding_rs2::mem::is_utf16_latin1(data),
        safe_encoding_rs_mem::is_utf16_latin1(data)
    );
}
...
fn fuzz_convert_utf8_to_utf16(data: &[u8]) {
    ...
    let len = encoding_rs2::mem::convert_utf8_to_utf16(data, &mut dst[..]);
...
fn fuzz_convert_str_to_utf16(data: &[u8]) {
    if let Ok(s) = std::str::from_utf8(data) {
        ...
        let mut dst = vec_with_len::<u16>(needed);
        let mut safe_dst = vec_with_len::<u16>(needed);
        let len = encoding_rs2::mem::convert_str_to_utf16(s, &mut dst[..]);
...
fn fuzz_convert_utf16_to_utf8(data: &[u16]) {
    ...
    let len = encoding_rs2::mem::convert_utf16_to_utf8(data, &mut dst[..]);
...
fn fuzz_convert_utf16_to_str(data: &[u16]) {
    ...
    let len = encoding_rs2::mem::convert_utf16_to_str(data, &mut dst[..]);
...
fn fuzz_convert_latin1_to_utf16(data: &[u8]) {
    ...
    let mut dst = vec_with_len::<u16>(needed);
    let mut safe_dst = vec_with_len::<u16>(needed);
    encoding_rs2::mem::convert_latin1_to_utf16(data, &mut dst[..]);
...
fn fuzz_convert_latin1_to_utf8(data: &[u8]) {
    ...
    let mut dst = vec_with_len::<u8>(needed);
    let mut safe_dst = vec_with_len::<u8>(needed);
    let len = encoding_rs2::mem::convert_latin1_to_utf8(data, &mut dst[..]);
...
fn fuzz_convert_latin1_to_str(data: &[u8]) {
    ...
    let mut dst = string_with_len(needed);
    let mut safe_dst = string_with_len(needed);
    let len = encoding_rs2::mem::convert_latin1_to_str(data, &mut dst[..]);
...
fn fuzz_convert_utf8_to_latin1_lossy(data: &[u8]) {
    ...
    let mut dst = vec_with_len::<u8>(needed);
    let mut safe_dst = vec_with_len::<u8>(needed);
    let len = encoding_rs2::mem::convert_utf8_to_latin1_lossy(data, &mut dst[..]);
...
fn fuzz_convert_utf16_to_latin1_lossy(data: &[u16]) {
    ...
    let mut dst = vec_with_len::<u8>(needed);
    let mut safe_dst = vec_with_len::<u8>(needed);
    encoding_rs2::mem::convert_utf16_to_latin1_lossy(data, &mut dst[..]);
...
fn fuzz_utf16_valid_up_to(data: &[u16]) {
    let up_to = encoding_rs2::mem::utf16_valid_up_to(data);
...
fn fuzz_ensure_utf16_validity(data: &[u16]) {
    ...
    encoding_rs2::mem::ensure_utf16_validity(&mut dst[..]);
...
fn fuzz_copy_ascii_to_ascii(data: &[u8]) {
    ...
    let mut dst = vec_with_len::<u8>(needed);
    let mut safe_dst = vec_with_len::<u8>(needed);
    let len = encoding_rs2::mem::copy_ascii_to_ascii(data, &mut dst[..]);
...
fn fuzz_copy_ascii_to_basic_latin(data: &[u8]) {
    ...
    let mut dst = vec_with_len::<u16>(needed);
    let mut safe_dst = vec_with_len::<u16>(needed);
    let len = encoding_rs2::mem::copy_ascii_to_basic_latin(data, &mut dst[..]);
...
fn fuzz_copy_basic_latin_to_ascii(data: &[u16]) {
    ...
    let mut dst = vec_with_len::<u8>(needed);
    let mut safe_dst = vec_with_len::<u8>(needed);
    let len = encoding_rs2::mem::copy_basic_latin_to_ascii(data, &mut dst[..]);
...
fn fuzz_is_utf8_bidi(data: &[u8]) {
    assert_eq!(
        encoding_rs2::mem::is_utf8_bidi(data),
        safe_encoding_rs_mem::is_utf8_bidi(data)
    );
}
...
fn fuzz_is_str_bidi(data: &[u8]) {
    if let Ok(s) = std::str::from_utf8(data) {
        assert_eq!(
            encoding_rs2::mem::is_str_bidi(s),
            safe_encoding_rs_mem::is_str_bidi(s)
        );
    }
}
...
fn fuzz_is_utf16_bidi(data: &[u16]) {
    assert_eq!(
        encoding_rs2::mem::is_utf16_bidi(data),
        safe_encoding_rs_mem::is_utf16_bidi(data)
    );
}
...
fn fuzz_check_utf8_for_latin1_and_bidi(data: &[u8]) {
    assert_eq!(
        encoding_rs2::mem::check_utf8_for_latin1_and_bidi(data),
        safe_encoding_rs_mem::check_utf8_for_latin1_and_bidi(data)
    );
}
...
fn fuzz_check_str_for_latin1_and_bidi(data: &[u8]) {
    if let Ok(s) = std::str::from_utf8(data) {
        assert_eq!(
            encoding_rs2::mem::check_str_for_latin1_and_bidi(s),
            safe_encoding_rs_mem::check_str_for_latin1_and_bidi(s)
        );
    }
}
...
fn fuzz_check_utf16_for_latin1_and_bidi(data: &[u16]) {
    assert_eq!(
        encoding_rs2::mem::check_utf16_for_latin1_and_bidi(data),
        safe_encoding_rs_mem::check_utf16_for_latin1_and_bidi(data)
    );
}
